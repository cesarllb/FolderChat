...
=======
from folderchat.constants import APP_NAME, PAGE_ICON, UPLOAD_HELP, USAGE_HELP
from folderchat.helper import (
...
with datasource_container:
    enable_monitoring_dir = st.checkbox(
        "Monitoring a folder", help="Select for choose the directory of your project and add any new files or changes on it."
    )

if enable_monitoring_dir:
    del data_source
    monitoring_dir = st.text_input(
        "Enter the path of your project directory",
        placeholder="Any path pointing to a directory of files",
    )
    event_loop = st.button('Stop directory scanning')
#---------------------------------------------------
...
if event_loop:
    st.session_state["event_loop"] = False

if monitoring_dir and monitoring_dir != st.session_state["data_source"]:
    logger.info(f"Monitoring directory provided: '{monitoring_dir}'")
    st.session_state["data_source"] = monitoring_dir
    st.session_state["event_loop"] = True
    update_chain_with_monitoring()
